...
class RequestsScheduleParser:
    def __init__(self, department_name: str, group_name: str):
        ...
        self.session.headers.update(settings.REQ_HEADERS)
    ...
    def _get_lessons_data(self):
        main_page_resp = self.session.get(f'{settings.SCHEDULE_URL}?alias=429')
        ...
        group_code_resp = self.session.post(f'{settings.SCHEDULE_URL}plugins/AutoRasp/SearchGroup.php',
                                            files=department_id)
